...
def predict_regression(data_dict: dict) -> float:
    """Predict arrival delay in minutes (regression)."""
...
def predict_classification(data_dict: dict) -> float:
    """Predict probability of flight delay (classification)."""
    df = pd.DataFrame([data_dict])
    df = encode_input(df)
    df = add_features(df)
    X = df[clf_features]
    ...
    y_proba = float(clf_model.predict(dtest)[0])  # probability of delay
